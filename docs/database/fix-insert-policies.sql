-- Fix INSERT policies that currently have NULL conditions

-- Admin entity tables INSERT policies
DROP POLICY IF EXISTS "Only admins can insert artists" ON artists;
CREATE POLICY "Only admins can insert artists" ON artists 
    FOR INSERT WITH CHECK ((auth.jwt() ->> 'role') = 'admin');

DROP POLICY IF EXISTS "Only admins can insert concerts" ON concerts;
CREATE POLICY "Only admins can insert concerts" ON concerts 
    FOR INSERT WITH CHECK ((auth.jwt() ->> 'role') = 'admin');

DROP POLICY IF EXISTS "Only admins can insert festivals" ON festivals;
CREATE POLICY "Only admins can insert festivals" ON festivals 
    FOR INSERT WITH CHECK ((auth.jwt() ->> 'role') = 'admin');

DROP POLICY IF EXISTS "Only admins can insert parks" ON parks;
...
-- Favorites tables INSERT policies
